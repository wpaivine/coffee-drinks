...
<div class="min-w-max w-1/2 m-auto">
	<div class="grid grid-cols-4 gap-1 min-w-min ">
		<div class="select-none">
			<span class="float-right font-body text-brown-900 text-2xl px-4"> drink size </span>
		</div>
		...
		{#each ingredientNames as coffeeIngredient, idx}
			<div class="select-none">
				<span class="float-right font-body text-brown-900 text-2xl select-none px-4">
					{coffeeIngredient.name}
				</span>
			</div>
			<div class="select-none col-span-2">
				<input
					class="ingredient float-left rounded-xl overflow-hidden appearance-none bg-brown-800 w-full drop-shadow-md"
					type="range"
